...
#include "CWRGameState.generated.h"
...
class UCWRExperienceManagerComponent;
class UObject;
struct FFrame;
 ...
 */
UCLASS(Config = Game)
class CWR_API ACWRGameState : public AModularGameStateBase, public IAbilitySystemInterface
{
	GENERATED_BODY()
	
	public:

	ACWRGameState(const FObjectInitializer& ObjectInitializer = FObjectInitializer::Get());
	...
	UFUNCTION(BlueprintCallable, Category = "CWR|GameState")
	UCWRAbilitySystemComponent* GetCWRAbilitySystemComponent() const { return AbilitySystemComponent; }

	// Send a message that all clients will (probably) get
	// (use only for client notifications like eliminations, server join messages, etc... that can handle being lost)
	UFUNCTION(NetMulticast, Unreliable, BlueprintCallable, Category = "CWR|GameState")
	void MulticastMessageToClients(const FCWRVerbMessage Message);

	// Send a message that all clients will be guaranteed to get
	// (use only for client notifications that cannot handle being lost)
	UFUNCTION(NetMulticast, Reliable, BlueprintCallable, Category = "CWR|GameState")
	void MulticastReliableMessageToClients(const FCWRVerbMessage Message);
...
private:
	...
	UPROPERTY()
	TObjectPtr<UCWRExperienceManagerComponent> ExperienceManagerComponent;

	// The ability system component subobject for game-wide things (primarily gameplay cues)
	UPROPERTY(VisibleAnywhere, Category = "CWR|GameState")
