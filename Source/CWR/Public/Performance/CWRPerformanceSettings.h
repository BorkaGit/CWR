...
#include "CWRPerformanceSettings.generated.h"

enum class ECWRDisplayablePerformanceStat : uint8;

class UObject;

// Describes one platform-specific device profile variant that the user can choose from in the UI
USTRUCT()
struct FCWRQualityDeviceProfileVariant
{
	GENERATED_BODY()
...
USTRUCT()
struct FCWRPerformanceStatGroup
{
	GENERATED_BODY()
...
class UCWRPlatformSpecificRenderingSettings : public UPlatformSettings
{
	GENERATED_BODY()

public:
	UCWRPlatformSpecificRenderingSettings();

	// Helper method to get the performance settings object, directed via platform settings
	static const UCWRPlatformSpecificRenderingSettings* Get();
...
public:
	...
	// Note that this will usually be set from platform-specific ini files, not via the UI
	UPROPERTY(EditAnywhere, Config, Category=DeviceProfiles)
	TArray<FCWRQualityDeviceProfileVariant> UserFacingDeviceProfileOptions;
	...
	UPROPERTY(EditAnywhere, Config, Category=VideoSettings)
	ECWRFramePacingMode FramePacingMode = ECWRFramePacingMode::DesktopStyle;

	// Potential frame rates to display for mobile
	// Note: This is further limited by CWR.DeviceProfile.Mobile.MaxFrameRate from the
	// platform-specific device profile and what the platform frame pacer reports as supported
	UPROPERTY(EditAnywhere, Config, Category=VideoSettings, meta=(EditCondition="FramePacingMode==ECWRFramePacingMode::MobileStyle", ForceUnits=Hz))
 ...
 */
UCLASS(config=Game, defaultconfig, meta=(DisplayName="CWR Performance Settings"))
class UCWRPerformanceSettings : public UDeveloperSettingsBackedByCVars
{
	GENERATED_BODY()

public:
	UCWRPerformanceSettings();
...
public:
	...
	TArray<FCWRPerformanceStatGroup> UserFacingPerformanceStats;
};
