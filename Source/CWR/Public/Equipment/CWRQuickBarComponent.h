...
class CWR_API UCWRQuickBarComponent : public UControllerComponent
{
	...
	int32 FindSlotIndex(TSubclassOf<UCWRInventoryItemDefinition> ItemDef);

	UFUNCTION(BlueprintCallable, BlueprintAuthorityOnly)
	UCWRInventoryItemInstance* GetItemAtSlot(int32 SlotIndex);

	UFUNCTION(BlueprintCallable, BlueprintAuthorityOnly)
	int32 FindBestPrioritySlotIndex();
