...
#include "CWRDeveloperSettings.generated.h"
...
class UCWRDeveloperSettings : public UDeveloperSettingsBackedByCVars
{
	GENERATED_BODY()
	
	public:
	UCWRDeveloperSettings();
...
public:
	...
	UPROPERTY(EditDefaultsOnly, BlueprintReadOnly, config, Category=CWR, meta=(AllowedTypes="CWRExperienceDefinition"))
	FPrimaryAssetId ExperienceOverride;

	UPROPERTY(EditDefaultsOnly, BlueprintReadOnly, config, Category=CWRBots, meta=(InlineEditConditionToggle))
	bool bOverrideBotCount = false;

	UPROPERTY(EditDefaultsOnly, BlueprintReadOnly, config, Category=CWRBots, meta=(EditCondition=bOverrideBotCount))
	int32 OverrideNumPlayerBotsToSpawn = 0;

	UPROPERTY(EditDefaultsOnly, BlueprintReadOnly, config, Category=CWRBots)
	...
	UPROPERTY(EditDefaultsOnly, BlueprintReadOnly, config, Category=CWR)
	bool bTestFullGameFlowInPIE = false;

	/**
	* Should force feedback effects be played, even if the last input device was not a gamepad?
	* The default behavior in CWR is to only play force feedback if the most recent input device was a gamepad.
	*/
	UPROPERTY(config, EditAnywhere, Category = CWR, meta = (ConsoleVariable = "CWRPC.ShouldAlwaysPlayForceFeedback"))
	...
	UPROPERTY(config, EditAnywhere, Category=CWR)
	TArray<FCWRCheatToRun> CheatsToRun;
...
public:
	...
	CWR_API void OnPlayInEditorStarted() const;
