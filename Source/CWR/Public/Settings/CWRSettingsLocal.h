...
#include "CWRSettingsLocal.generated.h"
...
enum class ECWRStatDisplayMode : uint8;

USTRUCT()
struct FCWRScalabilitySnapshot
{
	GENERATED_BODY()

	FCWRScalabilitySnapshot();
 ...
 */
UCLASS()
class UCWRSettingsLocal : public UGameUserSettings
{
	GENERATED_BODY()
...
public:
	...
	ECWRStatDisplayMode GetPerfStatDisplayState(ECWRDisplayablePerformanceStat Stat) const;
	
	/** Sets the display mode for the specified performance stat */
	void SetPerfStatDisplayState(ECWRDisplayablePerformanceStat Stat, ECWRStatDisplayMode DisplayMode);

	/** Fired when the display state for a performance stat has changed, or the settings are applied */
	DECLARE_EVENT(UCWRSettingsLocal, FPerfStatSettingsChanged);
...
private:
	...
	UPROPERTY(Config)
	TMap<ECWRDisplayablePerformanceStat, ECWRStatDisplayMode> DisplayStatList;
...
private:
	...
	void OverrideQualityLevelsToScalabilityMode(const FCWRScalabilitySnapshot& InMode, Scalability::FQualityLevels& InOutLevels);
...
private:
	...
	FCWRScalabilitySnapshot DeviceDefaultScalabilitySettings;
...
private:
	...
	// Audio - Volume
public:
	//DECLARE_EVENT_OneParam(UCWRSettingsLocal, FAudioDeviceChanged, const FString& /*DeviceId*/);
