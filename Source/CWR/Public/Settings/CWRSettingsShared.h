...
#include "CWRSettingsShared.generated.h"
...
UENUM(BlueprintType)
enum class ECWRAllowBackgroundAudioSetting : uint8
{
	Off,
	AllSounds,
...
UENUM(BlueprintType)
enum class ECWRGamepadSensitivity : uint8
{
...
/**
 * UCWRSettingsShared - The "Shared" settings are stored as part of the USaveGame system, these settings are not machine
 ...
 */
UCLASS()
class UCWRSettingsShared : public ULocalPlayerSaveGame
{
	GENERATED_BODY()
	
	public:
	DECLARE_EVENT_OneParam(UCWRSettingsShared, FOnSettingChangedEvent, UCWRSettingsShared* Settings);
...
public:
	...
	static UCWRSettingsShared* CreateTemporarySettings(const UCWRLocalPlayer* LocalPlayer);
	
	/** Synchronously loads a settings object, this is not valid to call before login */
	static UCWRSettingsShared* LoadOrCreateSettings(const UCWRLocalPlayer* LocalPlayer);

	DECLARE_DELEGATE_OneParam(FOnSettingsLoadedEvent, UCWRSettingsShared* Settings);

	/** Starts an async load of the settings object, calls Delegate on completion */
	static bool AsyncLoadOrCreateSettings(const UCWRLocalPlayer* LocalPlayer, FOnSettingsLoadedEvent Delegate);
...
public:
	UFUNCTION()
	ECWRAllowBackgroundAudioSetting GetAllowAudioInBackgroundSetting() const { return AllowAudioInBackground; }
	UFUNCTION()
	void SetAllowAudioInBackgroundSetting(ECWRAllowBackgroundAudioSetting NewValue);

	void ApplyBackgroundAudioSettings();

private:
	UPROPERTY()
	ECWRAllowBackgroundAudioSetting AllowAudioInBackground = ECWRAllowBackgroundAudioSetting::Off;
...
public:
	UFUNCTION()
	ECWRGamepadSensitivity GetGamepadLookSensitivityPreset() const { return GamepadLookSensitivityPreset; }
	UFUNCTION()
	void SetLookSensitivityPreset(ECWRGamepadSensitivity NewValue) { ChangeValueAndDirty(GamepadLookSensitivityPreset, NewValue); ApplyInputSensitivity(); }

	UFUNCTION()
	ECWRGamepadSensitivity GetGamepadTargetingSensitivityPreset() const { return GamepadTargetingSensitivityPreset; }
	UFUNCTION()
	void SetGamepadTargetingSensitivityPreset(ECWRGamepadSensitivity NewValue) { ChangeValueAndDirty(GamepadTargetingSensitivityPreset, NewValue); ApplyInputSensitivity(); }

	void ApplyInputSensitivity();
	
private:
	UPROPERTY()
	ECWRGamepadSensitivity GamepadLookSensitivityPreset = ECWRGamepadSensitivity::Normal;
	UPROPERTY()
	ECWRGamepadSensitivity GamepadTargetingSensitivityPreset = ECWRGamepadSensitivity::Normal;
