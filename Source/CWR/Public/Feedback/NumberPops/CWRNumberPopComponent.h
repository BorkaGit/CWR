...
#include "CWRNumberPopComponent.generated.h"

class UObject;
struct FFrame;

USTRUCT(BlueprintType)
struct FCWRNumberPopRequest
{
	GENERATED_BODY()

	// The world location to create the number pop at
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "CWR|Number Pops")
...
UCLASS(Abstract)
class CWR_API UCWRNumberPopComponent : public UControllerComponent
{
	GENERATED_BODY()
	
public:

	UCWRNumberPopComponent(const FObjectInitializer& ObjectInitializer = FObjectInitializer::Get());

	/** Adds a damage number to the damage number list for visualization */
	UFUNCTION(BlueprintCallable, Category = Foo)
	virtual void AddNumberPop(const FCWRNumberPopRequest& NewRequest) {}
};
