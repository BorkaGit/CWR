...
#include "CWRPlayerStart.generated.h"

class AController;
class UObject;

enum class ECWRPlayerStartLocationOccupancy
{
	Empty,
	Partial,
	Full
 ...
 */
UCLASS(Config = Game)
class CWR_API ACWRPlayerStart : public APlayerStart
{
	GENERATED_BODY()
	
public:
	ACWRPlayerStart(const FObjectInitializer& ObjectInitializer = FObjectInitializer::Get());

	const FGameplayTagContainer& GetGameplayTags() { return StartPointTags; }

	ECWRPlayerStartLocationOccupancy GetLocationOccupancy(AController* const ControllerPawnToFit) const;
