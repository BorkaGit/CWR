...
#include "CWRWeaponStateComponent.generated.h"
...
struct FCWRScreenSpaceHitLocation
{
...
struct FCWRServerSideHitMarkerBatch
{
	FCWRServerSideHitMarkerBatch() { }

	FCWRServerSideHitMarkerBatch(uint8 InUniqueId) :
		UniqueId(InUniqueId)
	{ }

	TArray<FCWRScreenSpaceHitLocation> Markers;
...
UCLASS()
class CWR_API UCWRWeaponStateComponent : public UControllerComponent
{
	GENERATED_BODY()
	
	public:

	UCWRWeaponStateComponent(const FObjectInitializer& ObjectInitializer = FObjectInitializer::Get());
	...
	void GetLastWeaponDamageScreenLocations(TArray<FCWRScreenSpaceHitLocation>& WeaponDamageScreenLocations)
	{
...
private:
	...
	TArray<FCWRScreenSpaceHitLocation> LastWeaponDamageScreenLocations;

	/** The unconfirmed hits */
	TArray<FCWRServerSideHitMarkerBatch> UnconfirmedServerSideHitMarkers;
