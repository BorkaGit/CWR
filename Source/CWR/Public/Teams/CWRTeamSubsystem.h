...
#include "CWRTeamSubsystem.generated.h"

class ACWRTeamSpawnPoint;
class AActor;
class ACWRPlayerState;
class ACWRTeamInfoBase;
...
DECLARE_DYNAMIC_MULTICAST_DELEGATE_OneParam(FOnCWRTeamDisplayAssetChangedDelegate, const UCWRTeamDisplayAsset*, DisplayAsset);

USTRUCT()
struct FCWRTeamTrackingInfo
{
	GENERATED_BODY()

public:
	UPROPERTY()
	TObjectPtr<ACWRTeamPublicInfo> PublicInfo = nullptr;

	UPROPERTY()
	TObjectPtr<ACWRTeamPrivateInfo> PrivateInfo = nullptr;

	UPROPERTY()
	TObjectPtr<UCWRTeamDisplayAsset> DisplayAsset = nullptr;

	UPROPERTY()
	FOnCWRTeamDisplayAssetChangedDelegate OnTeamDisplayAssetChanged;

public:
	void SetTeamInfo(ACWRTeamInfoBase* Info);
	void RemoveTeamInfo(ACWRTeamInfoBase* Info);
};

// Result of comparing the team affiliation for two actors
UENUM(BlueprintType)
enum class ECWRTeamComparison : uint8
{
...
UCLASS()
class CWR_API UCWRTeamSubsystem : public UWorldSubsystem
{
	GENERATED_BODY()
	...
	bool RegisterTeamInfo(ACWRTeamInfoBase* TeamInfo);

	// Tries to unregister a team, will return false if it didn't work
	bool UnregisterTeamInfo(ACWRTeamInfoBase* TeamInfo);
	...
	const ACWRPlayerState* FindPlayerStateFromActor(const AActor* PossibleTeamActor) const;
	...
	ECWRTeamComparison CompareTeams(const UObject* A, const UObject* B, int32& TeamIdA, int32& TeamIdB) const;

	// Compare the teams of two actors and returns a value indicating if they are on same teams, different teams, or one/both are invalid
	ECWRTeamComparison CompareTeams(const UObject* A, const UObject* B) const;
	...
	UFUNCTION(BlueprintCallable, Category=Teams)
	UCWRTeamDisplayAsset* GetTeamDisplayAsset(int32 TeamId, int32 ViewerTeamId);

	// Gets the team display asset for the specified team, from the perspective of the specified team
	// (You have to specify a viewer too, in case the game mode is in a 'local player is always blue team' sort of situation)
	UFUNCTION(BlueprintCallable, Category = Teams)
	UCWRTeamDisplayAsset* GetEffectiveTeamDisplayAsset(int32 TeamId, UObject* ViewerTeamAgent);
	...
	void NotifyTeamDisplayAssetModified(UCWRTeamDisplayAsset* ModifiedAsset);

	// Register for a team display asset notification for the specified team ID
	FOnCWRTeamDisplayAssetChangedDelegate& GetTeamDisplayAssetChangedDelegate(int32 TeamId);

	UFUNCTION(BlueprintCallable)
	TArray<ACWRTeamSpawnPoint*> GetCapturedSpawnPoints(const int32 TeamId);

	UFUNCTION(BlueprintCallable)
	void CaptureSpawnPoint(ACWRTeamSpawnPoint* SpawnPoint, const int32 TeamId);
	
private:
	UPROPERTY()
	TMap<int32, FCWRTeamTrackingInfo> TeamMap;
