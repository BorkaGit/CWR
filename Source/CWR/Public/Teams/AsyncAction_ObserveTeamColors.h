...
class ICWRTeamAgentInterface;
class UCWRTeamDisplayAsset;
struct FFrame;

DECLARE_DYNAMIC_MULTICAST_DELEGATE_ThreeParams(FTeamColorObservedAsyncDelegate, bool, bTeamSet, int32, TeamId, const UCWRTeamDisplayAsset*, DisplayAsset);
...
public:
	...
	//  - For anything that can ever belong to a team (implements ICWRTeamAgentInterface),
...
private:
	void BroadcastChange(int32 NewTeam, const UCWRTeamDisplayAsset* DisplayAsset);

	UFUNCTION()
	void OnWatchedAgentChangedTeam(UObject* TeamAgent, int32 OldTeam, int32 NewTeam);

	UFUNCTION()
	void OnDisplayAssetChanged(const UCWRTeamDisplayAsset* DisplayAsset);

	TWeakInterfacePtr<ICWRTeamAgentInterface> TeamInterfacePtr;
