...
UCWRGameplayAbility_Weapon_Fire::UCWRGameplayAbility_Weapon_Fire(const FObjectInitializer& ObjectInitializer)
	: Super(ObjectInitializer)
{
	SourceBlockedTags.AddTag(TAG_WeaponFireBlocked);
}

void UCWRGameplayAbility_Weapon_Fire::OnGiveAbility(const FGameplayAbilityActorInfo* ActorInfo, const FGameplayAbilitySpec& Spec)
{
	Super::OnGiveAbility(ActorInfo, Spec);

	FGameplayTag FireModeTag;
	
	switch (GetWeaponInstance()->GetFireMode())
	{
	case ECWRFireMode::Auto:
		FireModeTag = FCWRGameplayTags::Get().Weapon_FireMode_Auto;
		break;

	case ECWRFireMode::Single:
		FireModeTag = FCWRGameplayTags::Get().Weapon_FireMode_Single;
		break;
	...
	if ( !GetAbilitySystemComponentFromActorInfo()->HasMatchingGameplayTag(FireModeTag) )
	{
		UAbilitySystemBlueprintLibrary::AddLooseGameplayTags(GetAvatarActorFromActorInfo(),UBlueprintGameplayTagLibrary::MakeGameplayTagContainerFromTag(FireModeTag), true);
	}
}
...
ECollisionChannel UCWRGameplayAbility_Weapon_Fire::DetermineTraceChannel(FCollisionQueryParams& TraceParams, bool bIsSimulated) const
{
	return CWR_TraceChannel_Weapon;
}
