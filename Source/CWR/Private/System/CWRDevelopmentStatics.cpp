...
#include "System/CWRDevelopmentStatics.h"
...
bool UCWRDevelopmentStatics::ShouldSkipDirectlyToGameplay()
{
#if WITH_EDITOR
	if (GIsEditor)
	{
		return !GetDefault<UCWRDeveloperSettings>()->bTestFullGameFlowInPIE;
	}
#endif
	return false;
}

bool UCWRDevelopmentStatics::ShouldLoadCosmeticBackgrounds()
{
#if WITH_EDITOR
	if (GIsEditor)
	{
		return !GetDefault<UCWRDeveloperSettings>()->bSkipLoadingCosmeticBackgroundsInPIE;
	}
#endif
	return true;
}

bool UCWRDevelopmentStatics::CanPlayerBotsAttack()
{
#if WITH_EDITOR
	if (GIsEditor)
	{
		return GetDefault<UCWRDeveloperSettings>()->bAllowPlayerBotsToAttack;
	}
#endif
	return true;
}
...
UWorld* UCWRDevelopmentStatics::FindPlayInEditorAuthorityWorld()
{
	check(GEngine);
...
TArray<FAssetData> UCWRDevelopmentStatics::GetAllBlueprints()
{
...
UClass* UCWRDevelopmentStatics::FindBlueprintClass(const FString& TargetNameRaw, UClass* DesiredBaseClass)
{
	FString TargetName = TargetNameRaw;
	TargetName.RemoveFromEnd(TEXT("_C"), ESearchCase::CaseSensitive);

	TArray<FAssetData> BlueprintList = UCWRDevelopmentStatics::GetAllBlueprints();
...
UClass* UCWRDevelopmentStatics::FindClassByShortName(const FString& SearchToken, UClass* DesiredBaseClass, bool bLogFailures)
{
