...
namespace CWR
{
	...
	static FAutoConsoleVariableRef CVarCWRTestEncryption(
		TEXT("CWR.TestEncryption"),
		bTestEncryption,
	...
	static FAutoConsoleVariableRef CVarCWRUseDTLSEncryption(
		TEXT("CWR.UseDTLSEncryption"),
		bUseDTLSEncryption,
		TEXT("Set to true if using CWR.TestEncryption and the DTLS packet handler."),
		ECVF_Default);
	...
	static FAutoConsoleVariableRef CVarCWRTestDTLSFingerprint(
		TEXT("CWR.TestDTLSFingerprint"),
...
UCWRGameInstance::UCWRGameInstance(const FObjectInitializer& ObjectInitializer)
	: Super(ObjectInitializer)
{
}

void UCWRGameInstance::Init()
{
	Super::Init();
	...
	{
		ComponentManager->RegisterInitState(FCWRGameplayTags::Get().InitState_Spawned, false, FGameplayTag());
		ComponentManager->RegisterInitState(FCWRGameplayTags::Get().InitState_DataAvailable, false, FCWRGameplayTags::Get().InitState_Spawned);
		ComponentManager->RegisterInitState(FCWRGameplayTags::Get().InitState_DataInitialized, false, FCWRGameplayTags::Get().InitState_DataAvailable);
		ComponentManager->RegisterInitState(FCWRGameplayTags::Get().InitState_GameplayReady, false, FCWRGameplayTags::Get().InitState_DataInitialized);
	}
	...
	{
		SessionSubsystem->OnPreClientTravelEvent.AddUObject(this, &UCWRGameInstance::OnPreClientTravelToSession);
	}
}

void UCWRGameInstance::Shutdown()
{
	if (UCommonSessionSubsystem* SessionSubsystem = GetSubsystem<UCommonSessionSubsystem>())
	{
...
ACWRPlayerController* UCWRGameInstance::GetPrimaryPlayerController() const
{
	return Cast<ACWRPlayerController>(Super::GetPrimaryPlayerController(false));
}

bool UCWRGameInstance::CanJoinRequestedSession() const
{
...
void UCWRGameInstance::HandlerUserInitialized(const UCommonUserInfo* UserInfo, bool bSuccess, FText Error, ECommonUserPrivilege RequestedPrivilege, ECommonUserOnlineContext OnlineContext)
{
	...
	{
		UCWRLocalPlayer* LocalPlayer = Cast<UCWRLocalPlayer>(GetLocalPlayerByIndex(UserInfo->LocalPlayerIndex));
...
void UCWRGameInstance::ReceivedNetworkEncryptionToken(const FString& EncryptionToken, const FOnEncryptionKeyResponse& Delegate)
{
	...
	{
#if UE_WITH_DTLS
		if (CWR::bUseDTLSEncryption)
		{
			TSharedPtr<FDTLSCertificate> Cert;

			if (CWR::bTestDTLSFingerprint)
			{
			...
			else
			{
				...
				const FString CertPath = FPaths::ProjectContentDir() / TEXT("DTLS") / TEXT("CWRTest.pem");
...
void UCWRGameInstance::ReceivedNetworkEncryptionAck(const FOnEncryptionKeyResponse& Delegate)
{
	// This is a simple implementation to demonstrate using encryption for game traffic using a hardcoded key.
	// For a complete implementation, you would likely want to retrieve the encryption key from a secure source,
	// such as from a web service over HTTPS. This could be done in this function, even asynchronously - just
...
void UCWRGameInstance::OnPreClientTravelToSession(FString& URL)
{
	// Add debug encryption token if desired.
	if (CWR::bTestEncryption)
	{
#if UE_WITH_DTLS
		if (CWR::bUseDTLSEncryption)
		{
