...
namespace CWRGameplayCueManagerCvars
{
	static FAutoConsoleCommand CVarDumpGameplayCues(
		TEXT("CWR.DumpGameplayCues"),
		TEXT("Shows all assets that were loaded via CWRGameplayCueManager and are currently in memory."),
		FConsoleCommandWithArgsDelegate::CreateStatic(UCWRGameplayCueManager::DumpGameplayCues));

	static ECWREditorLoadMode LoadMode = ECWREditorLoadMode::LoadUpfront;
}
...
UCWRGameplayCueManager::UCWRGameplayCueManager(const FObjectInitializer& ObjectInitializer)
	: Super(ObjectInitializer)
{
}

UCWRGameplayCueManager* UCWRGameplayCueManager::Get()
{
	return Cast<UCWRGameplayCueManager>(UAbilitySystemGlobals::Get().GetGameplayCueManager());
}

void UCWRGameplayCueManager::OnCreated()
{
	Super::OnCreated();
...
void UCWRGameplayCueManager::LoadAlwaysLoadedCues()
{
...
bool UCWRGameplayCueManager::ShouldAsyncLoadRuntimeObjectLibraries() const
{
	switch (CWRGameplayCueManagerCvars::LoadMode)
	{
	case ECWREditorLoadMode::LoadUpfront:
		return true;
	case ECWREditorLoadMode::PreloadAsCuesAreReferenced_GameOnly:
#if WITH_EDITOR
		if (GIsEditor)
		{
			return false;
...
#endif
		break;
	case ECWREditorLoadMode::PreloadAsCuesAreReferenced:
		break;
	}
...
bool UCWRGameplayCueManager::ShouldSyncLoadMissingGameplayCues() const
{
	return false;
}

bool UCWRGameplayCueManager::ShouldAsyncLoadMissingGameplayCues() const
{
	return true;
}

void UCWRGameplayCueManager::DumpGameplayCues(const TArray<FString>& Args)
{
	UCWRGameplayCueManager* GCM = Cast<UCWRGameplayCueManager>(UAbilitySystemGlobals::Get().GetGameplayCueManager());
	if (!GCM)
	{
		return;
	}
...
void UCWRGameplayCueManager::OnGameplayTagLoaded(const FGameplayTag& Tag)
{
		...
		TGraphTask<FGameplayCueTagThreadSynchronizeGraphTask>::CreateTask().ConstructAndDispatchWhenReady([]()
			{
				if (GIsRunning)
				{
					if (UCWRGameplayCueManager* StrongThis = Get())
					{
...
void UCWRGameplayCueManager::HandlePostGarbageCollect()
{
...
void UCWRGameplayCueManager::ProcessLoadedTags()
{
...
void UCWRGameplayCueManager::ProcessTagToPreload(const FGameplayTag& Tag, UObject* OwningObject)
{
	switch (CWRGameplayCueManagerCvars::LoadMode)
	{
	case ECWREditorLoadMode::LoadUpfront:
...
void UCWRGameplayCueManager::OnPreloadCueComplete(FSoftObjectPath Path, TWeakObjectPtr<UObject> OwningObject, bool bAlwaysLoadedCue)
{
...
void UCWRGameplayCueManager::RegisterPreloadedCue(UClass* LoadedGameplayCueClass, UObject* OwningObject)
{
...
void UCWRGameplayCueManager::HandlePostLoadMap(UWorld* NewWorld)
{
	if (RuntimeGameplayCueObjectLibrary.CueSet)
	{
...
void UCWRGameplayCueManager::UpdateDelayLoadDelegateListeners()
{
...
bool UCWRGameplayCueManager::ShouldDelayLoadGameplayCues() const
{
...
void UCWRGameplayCueManager::RefreshGameplayCuePrimaryAsset()
{
