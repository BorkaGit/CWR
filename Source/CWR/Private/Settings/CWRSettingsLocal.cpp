...
#include "Development/CWRPlatformEmulationSettings.h"
//#include "SoundControlBus.h"
#include "EnhancedActionKeyMapping.h"


#if WITH_EDITOR
static TAutoConsoleVariable<bool> CVarApplyFrameRateSettingsInPIE(TEXT("CWR.Settings.ApplyFrameRateSettingsInPIE"),
	false,
	TEXT("Should we apply frame rate settings in PIE?"),
	ECVF_Default);

static TAutoConsoleVariable<bool> CVarApplyFrontEndPerformanceOptionsInPIE(TEXT("CWR.Settings.ApplyFrontEndPerformanceOptionsInPIE"),
	false,
	TEXT("Do we apply front-end specific performance options in PIE?"),
	ECVF_Default);

static TAutoConsoleVariable<bool> CVarApplyDeviceProfilesInPIE(TEXT("CWR.Settings.ApplyDeviceProfilesInPIE"),
	false,
...
static TAutoConsoleVariable<int32> CVarDeviceProfileDrivenTargetFps(
	TEXT("CWR.DeviceProfile.Console.TargetFPS"),
	-1,
...
static TAutoConsoleVariable<int32> CVarDeviceProfileDrivenFrameSyncType(
	TEXT("CWR.DeviceProfile.Console.FrameSyncType"),
	-1,
...
static TAutoConsoleVariable<int32> CVarDeviceProfileDrivenMobileDefaultFrameRate(
	TEXT("CWR.DeviceProfile.Mobile.DefaultFrameRate"),
	30,
...
static TAutoConsoleVariable<int32> CVarDeviceProfileDrivenMobileMaxFrameRate(
	TEXT("CWR.DeviceProfile.Mobile.MaxFrameRate"),
	30,
...
static TAutoConsoleVariable<FString> CVarMobileQualityLimits(
	TEXT("CWR.DeviceProfile.Mobile.OverallQualityLimits"),
	TEXT(""),
...
static TAutoConsoleVariable<FString> CVarMobileResolutionQualityLimits(
	TEXT("CWR.DeviceProfile.Mobile.ResolutionQualityLimits"),
	TEXT(""),
...
static TAutoConsoleVariable<FString> CVarMobileResolutionQualityRecommendation(
	TEXT("CWR.DeviceProfile.Mobile.ResolutionQualityRecommendation"),
	TEXT("0:75"),
...
FCWRScalabilitySnapshot::FCWRScalabilitySnapshot()
{
...
namespace CWRSettingsHelpers
{
	...
	void FillScalabilitySettingsFromDeviceProfile(FCWRScalabilitySnapshot& Mode, const FString& Suffix = FString())
	{
		static_assert(sizeof(Scalability::FQualityLevels) == 88, "This function may need to be updated to account for new members");
	...
	int32 ConstrainFrameRateToBeCompatibleWithOverallQuality(int32 FrameRate, int32 OverallQuality)
	{
		...
		int32 LimitIndex = PossibleRates.FindLastByPredicate([=](const int32& TestRate)
		{
			...
			const bool bIsSupported = UCWRSettingsLocal::IsSupportedMobileFramePace(TestRate);

			return bAtOrBelowDesiredRate && bQualityDoesntExceedLimit && bIsSupported;
		});

		return PossibleRates.IsValidIndex(LimitIndex) ? PossibleRates[LimitIndex] : UCWRSettingsLocal::GetDefaultMobileFrameRate();
	}
...
UCWRSettingsLocal::UCWRSettingsLocal()
{
...
void UCWRSettingsLocal::SetToDefaults()
{
	Super::SetToDefaults();
...
void UCWRSettingsLocal::LoadSettings(bool bForceReload)
{
	Super::LoadSettings(bForceReload);

	// Console platforms use rhi.SyncInterval to limit framerate
	const UCWRPlatformSpecificRenderingSettings* PlatformSettings = UCWRPlatformSpecificRenderingSettings::Get();
	if (PlatformSettings->FramePacingMode == ECWRFramePacingMode::ConsoleStyle)
	{
		FrameRateLimit = 0.0f;
	}
	...
	CWRSettingsHelpers::FillScalabilitySettingsFromDeviceProfile(DeviceDefaultScalabilitySettings);
...
void UCWRSettingsLocal::ResetToCurrentSettings()
{
...
void UCWRSettingsLocal::BeginDestroy()
{
...
UCWRSettingsLocal* UCWRSettingsLocal::Get()
{
	return GEngine ? CastChecked<UCWRSettingsLocal>(GEngine->GetGameUserSettings()) : nullptr;
}

void UCWRSettingsLocal::ConfirmVideoMode()
{
...
float UCWRSettingsLocal::GetEffectiveFrameRateLimit()
{
	const UCWRPlatformSpecificRenderingSettings* PlatformSettings = UCWRPlatformSpecificRenderingSettings::Get();
	...
	if (PlatformSettings->FramePacingMode == ECWRFramePacingMode::DesktopStyle)
	{
...
int32 UCWRSettingsLocal::GetHighestLevelOfAnyScalabilityChannel() const
{
	return CWRSettingsHelpers::GetHighestLevelOfAnyScalabilityChannel(ScalabilityQuality);
}

void UCWRSettingsLocal::OverrideQualityLevelsToScalabilityMode(const FCWRScalabilitySnapshot& InMode, Scalability::FQualityLevels& InOutLevels)
{
	static_assert(sizeof(Scalability::FQualityLevels) == 88, "This function may need to be updated to account for new members");
...
void UCWRSettingsLocal::ClampQualityLevelsToDeviceProfile(const Scalability::FQualityLevels& ClampLevels, Scalability::FQualityLevels& InOutLevels)
{
	static_assert(sizeof(Scalability::FQualityLevels) == 88, "This function may need to be updated to account for new members");
...
void UCWRSettingsLocal::OnExperienceLoaded()
{
	ReapplyThingsDueToPossibleDeviceProfileChange();
}

void UCWRSettingsLocal::OnHotfixDeviceProfileApplied()
{
	ReapplyThingsDueToPossibleDeviceProfileChange();
}

void UCWRSettingsLocal::ReapplyThingsDueToPossibleDeviceProfileChange()
{
	ApplyNonResolutionSettings();
}

void UCWRSettingsLocal::SetShouldUseFrontendPerformanceSettings(bool bInFrontEnd)
{
	bInFrontEndForPerformancePurposes = bInFrontEnd;
	UpdateEffectiveFrameRateLimit();
}

bool UCWRSettingsLocal::ShouldUseFrontendPerformanceSettings() const
{
#if WITH_EDITOR
...
ECWRStatDisplayMode UCWRSettingsLocal::GetPerfStatDisplayState(ECWRDisplayablePerformanceStat Stat) const
{
	if (const ECWRStatDisplayMode* pMode = DisplayStatList.Find(Stat))
	{
		return *pMode;
	}
	else
	{
		return ECWRStatDisplayMode::Hidden;
	}
}

void UCWRSettingsLocal::SetPerfStatDisplayState(ECWRDisplayablePerformanceStat Stat, ECWRStatDisplayMode DisplayMode)
{
	if (DisplayMode == ECWRStatDisplayMode::Hidden)
	{
...
float UCWRSettingsLocal::GetDisplayGamma() const
{
	return DisplayGamma;
}

void UCWRSettingsLocal::SetDisplayGamma(float InGamma)
{
	DisplayGamma = InGamma;
	ApplyDisplayGamma();
}

void UCWRSettingsLocal::ApplyDisplayGamma()
{
	if (GEngine)
	{
...
float UCWRSettingsLocal::GetFrameRateLimit_OnBattery() const
{
	return FrameRateLimit_OnBattery;
}

void UCWRSettingsLocal::SetFrameRateLimit_OnBattery(float NewLimitFPS)
{
	FrameRateLimit_OnBattery = NewLimitFPS;
	UpdateEffectiveFrameRateLimit();
}

float UCWRSettingsLocal::GetFrameRateLimit_InMenu() const
{
	return FrameRateLimit_InMenu;
}

void UCWRSettingsLocal::SetFrameRateLimit_InMenu(float NewLimitFPS)
{
	FrameRateLimit_InMenu = NewLimitFPS;
	UpdateEffectiveFrameRateLimit();
}

float UCWRSettingsLocal::GetFrameRateLimit_WhenBackgrounded() const
{
	return FrameRateLimit_WhenBackgrounded;
}

void UCWRSettingsLocal::SetFrameRateLimit_WhenBackgrounded(float NewLimitFPS)
{
	FrameRateLimit_WhenBackgrounded = NewLimitFPS;
	UpdateEffectiveFrameRateLimit();
}

float UCWRSettingsLocal::GetFrameRateLimit_Always() const
{
	return GetFrameRateLimit();
}

void UCWRSettingsLocal::SetFrameRateLimit_Always(float NewLimitFPS)
{
	SetFrameRateLimit(NewLimitFPS);
	UpdateEffectiveFrameRateLimit();
}

void UCWRSettingsLocal::UpdateEffectiveFrameRateLimit()
{
...
int32 UCWRSettingsLocal::GetDefaultMobileFrameRate()
{
	return CVarDeviceProfileDrivenMobileDefaultFrameRate.GetValueOnGameThread();
}

int32 UCWRSettingsLocal::GetMaxMobileFrameRate()
{
	return CVarDeviceProfileDrivenMobileMaxFrameRate.GetValueOnGameThread();
}

bool UCWRSettingsLocal::IsSupportedMobileFramePace(int32 TestFPS)
{
...
int32 UCWRSettingsLocal::GetFirstFrameRateWithQualityLimit() const
{
	return CWRSettingsHelpers::GetFirstFrameRateWithQualityLimit();
}

int32 UCWRSettingsLocal::GetLowestQualityWithFrameRateLimit() const
{
	return CWRSettingsHelpers::GetLowestQualityWithFrameRateLimit();
}

void UCWRSettingsLocal::ResetToMobileDeviceDefaults()
{
	// Reset frame rate
...
int32 UCWRSettingsLocal::GetMaxSupportedOverallQualityLevel() const
{
	const UCWRPlatformSpecificRenderingSettings* PlatformSettings = UCWRPlatformSpecificRenderingSettings::Get();
	if ((PlatformSettings->FramePacingMode == ECWRFramePacingMode::MobileStyle) && DeviceDefaultScalabilitySettings.bHasOverrides)
	{
		return CWRSettingsHelpers::GetHighestLevelOfAnyScalabilityChannel(DeviceDefaultScalabilitySettings.Qualities);
	}
	else
	{
		return 3;
...
void UCWRSettingsLocal::SetMobileFPSMode(int32 NewLimitFPS)
{
	const UCWRPlatformSpecificRenderingSettings* PlatformSettings = UCWRPlatformSpecificRenderingSettings::Get();
	if (PlatformSettings->FramePacingMode == ECWRFramePacingMode::MobileStyle)
	{
...
void UCWRSettingsLocal::SetDesiredMobileFrameRateLimit(int32 NewLimitFPS)
{
...
void UCWRSettingsLocal::ClampMobileFPSQualityLevels(bool bWriteBack)
{
	const UCWRPlatformSpecificRenderingSettings* PlatformSettings = UCWRPlatformSpecificRenderingSettings::Get();
	if (PlatformSettings->FramePacingMode == ECWRFramePacingMode::MobileStyle)
	{
		const int32 QualityLimit = CWRSettingsHelpers::GetApplicableOverallQualityLimit(DesiredMobileFrameRateLimit);
...
void UCWRSettingsLocal::ClampMobileQuality()
{
	const UCWRPlatformSpecificRenderingSettings* PlatformSettings = UCWRPlatformSpecificRenderingSettings::Get();
	if (PlatformSettings->FramePacingMode == ECWRFramePacingMode::MobileStyle)
	{
...
void UCWRSettingsLocal::ClampMobileResolutionQuality(int32 TargetFPS)
{
	// Clamp mobile resolution quality
	float MaxMobileResQuality = CWRSettingsHelpers::GetApplicableResolutionQualityLimit(TargetFPS);
...
void UCWRSettingsLocal::RemapMobileResolutionQuality(int32 FromFPS, int32 ToFPS)
{
	...
	float MaxScaleValue = 0.0f;
	GetResolutionScaleInformationEx(CurrentScaleNormalized, CurrentScaleValue, MinScaleValue, MaxScaleValue);
	float FromMaxMobileResQuality = CWRSettingsHelpers::GetApplicableResolutionQualityLimit(FromFPS);
	float ToMaxMobileResQuality = CWRSettingsHelpers::GetApplicableResolutionQualityLimit(ToFPS);
...
FString UCWRSettingsLocal::GetDesiredDeviceProfileQualitySuffix() const
{
	return DesiredUserChosenDeviceProfileSuffix;
}

void UCWRSettingsLocal::SetDesiredDeviceProfileQualitySuffix(const FString& InDesiredSuffix)
{
	DesiredUserChosenDeviceProfileSuffix = InDesiredSuffix;
}
/*
void UCWRSettingsLocal::SetHeadphoneModeEnabled(bool bEnabled)
{
...
bool UCWRSettingsLocal::IsHeadphoneModeEnabled() const
{
	return bUseHeadphoneMode;
}

bool UCWRSettingsLocal::CanModifyHeadphoneModeEnabled() const
{
	static IConsoleVariable* BinauralSpatializationDisabledCVar = IConsoleManager::Get().FindConsoleVariable(TEXT("au.DisableBinauralSpatialization"));
	...
	return true;*/
/*	const bool bBinauralSettingControlledByOS = CWRSettingsHelpers::HasPlatformTrait(TAG_Platform_Trait_BinauralSettingControlledByOS);

	return bHRTFOptionAvailable && !bBinauralSettingControlledByOS;
}*/
/*
bool UCWRSettingsLocal::IsHDRAudioModeEnabled() const
{
	return bUseHDRAudioMode;
}*/
/*
void UCWRSettingsLocal::SetHDRAudioModeEnabled(bool bEnabled)
{
	...
	if (GEngine)
	{
		...
		{
		/	if (UCWRAudioMixEffectsSubsystem* CWRAudioMixEffectsSubsystem = World->GetSubsystem<UCWRAudioMixEffectsSubsystem>())
			{
				CWRAudioMixEffectsSubsystem->ApplyDynamicRangeEffectsChains(bEnabled);
			}
		}
	}
}*/

bool UCWRSettingsLocal::CanRunAutoBenchmark() const
{
	const UCWRPlatformSpecificRenderingSettings* PlatformSettings = UCWRPlatformSpecificRenderingSettings::Get();
	return PlatformSettings->bSupportsAutomaticVideoQualityBenchmark;
}

bool UCWRSettingsLocal::ShouldRunAutoBenchmarkAtStartup() const
{
...
void UCWRSettingsLocal::RunAutoBenchmark(bool bSaveImmediately)
{
...
void UCWRSettingsLocal::ApplyScalabilitySettings()
{
	Scalability::SetQualityLevels(ScalabilityQuality);
}
/*
float UCWRSettingsLocal::GetOverallVolume() const
{
	return OverallVolume;
}

void UCWRSettingsLocal::SetOverallVolume(float InVolume)
{
...
float UCWRSettingsLocal::GetMusicVolume() const
{
	return MusicVolume;
}

void UCWRSettingsLocal::SetMusicVolume(float InVolume)
{
	// Cache the incoming volume value
...
float UCWRSettingsLocal::GetSoundFXVolume() const
{
	return SoundFXVolume;
}

void UCWRSettingsLocal::SetSoundFXVolume(float InVolume)
{
	// Cache the incoming volume value
...
float UCWRSettingsLocal::GetDialogueVolume() const
{
	return DialogueVolume;
}

void UCWRSettingsLocal::SetDialogueVolume(float InVolume)
{
	// Cache the incoming volume value
...
float UCWRSettingsLocal::GetVoiceChatVolume() const
{
	return VoiceChatVolume;
}

void UCWRSettingsLocal::SetVoiceChatVolume(float InVolume)
{
	// Cache the incoming volume value
...
void UCWRSettingsLocal::SetVolumeForControlBus(USoundControlBus* InSoundControlBus, float InVolume)
{
	// Check to see if references to the control buses and control bus mixes have been loaded yet
...
//}
/*
void UCWRSettingsLocal::SetAudioOutputDeviceId(const FString& InAudioOutputDeviceId)
{
	AudioOutputDeviceId = InAudioOutputDeviceId;
	OnAudioOutputDeviceChanged.Broadcast(InAudioOutputDeviceId);
}*/

void UCWRSettingsLocal::ApplySafeZoneScale()
{
	SSafeZone::SetGlobalSafeZoneScale(GetSafeZone());
}

void UCWRSettingsLocal::ApplyNonResolutionSettings()
{
...
int32 UCWRSettingsLocal::GetOverallScalabilityLevel() const
{
...
void UCWRSettingsLocal::SetOverallScalabilityLevel(int32 Value)
{
	...
	if (PlatformSettings->FramePacingMode == ECWRFramePacingMode::MobileStyle)
	{
		...
		const int32 ConstrainedFrameRateLimit = CWRSettingsHelpers::ConstrainFrameRateToBeCompatibleWithOverallQuality(DesiredMobileFrameRateLimit, Value);
...
void UCWRSettingsLocal::SetControllerPlatform(const FName InControllerPlatform)
{
...
FName UCWRSettingsLocal::GetControllerPlatform() const
{
	return ControllerPlatform;
}

void UCWRSettingsLocal::LoadUserControlBusMix()
{
	if (GEngine)
	{
		if (const UWorld* World = GEngine->GetCurrentPlayWorld())
		{/*
			if (const UCWRAudioSettings* CWRAudioSettings = GetDefault<UCWRAudioSettings>())
			{
				...
				if (UObject* ObjPath = CWRAudioSettings->OverallVolumeControlBus.TryLoad())
				{
					...
					{
						ensureMsgf(SoundControlBus, TEXT("Overall Control Bus reference missing from CWR Audio Settings."));
					}
				}

				if (UObject* ObjPath = CWRAudioSettings->MusicVolumeControlBus.TryLoad())
				{
					if (USoundControlBus* SoundControlBus = Cast<USoundControlBus>(ObjPath))
					{
					...
					{
						ensureMsgf(SoundControlBus, TEXT("Music Control Bus reference missing from CWR Audio Settings."));
					}
				}

				if (UObject* ObjPath = CWRAudioSettings->SoundFXVolumeControlBus.TryLoad())
				{
					if (USoundControlBus* SoundControlBus = Cast<USoundControlBus>(ObjPath))
					{
					...
					{
						ensureMsgf(SoundControlBus, TEXT("SoundFX Control Bus reference missing from CWR Audio Settings."));
					}
				}

				if (UObject* ObjPath = CWRAudioSettings->DialogueVolumeControlBus.TryLoad())
				{
					if (USoundControlBus* SoundControlBus = Cast<USoundControlBus>(ObjPath))
					{
					...
					{
						ensureMsgf(SoundControlBus, TEXT("Dialogue Control Bus reference missing from CWR Audio Settings."));
					}
				}

				if (UObject* ObjPath = CWRAudioSettings->VoiceChatVolumeControlBus.TryLoad())
				{
					if (USoundControlBus* SoundControlBus = Cast<USoundControlBus>(ObjPath))
					{
					...
					{
						ensureMsgf(SoundControlBus, TEXT("VoiceChat Control Bus reference missing from CWR Audio Settings."));
					}
				}

				if (UObject* ObjPath = CWRAudioSettings->UserSettingsControlBusMix.TryLoad())
				{
					...
					{
						ensureMsgf(SoundControlBusMix, TEXT("User Settings Control Bus Mix reference missing from CWR Audio Settings."));
					}
				}
			}*/
		}
...
void UCWRSettingsLocal::OnAppActivationStateChanged(bool bIsActive)
{
	// We might want to adjust the frame rate when the app loses/gains focus on multi-window platforms
	UpdateEffectiveFrameRateLimit();
}

void UCWRSettingsLocal::UpdateGameModeDeviceProfileAndFps()
{
#if WITH_EDITOR
	...
	const UCWRPlatformSpecificRenderingSettings* PlatformSettings = UCWRPlatformSpecificRenderingSettings::Get();
	const TArray<FCWRQualityDeviceProfileVariant>& UserFacingVariants = PlatformSettings->UserFacingDeviceProfileOptions;
	...
	int32 SuffixIndex = UserFacingVariants.IndexOfByPredicate([&](const FCWRQualityDeviceProfileVariant& Data){ return Data.DeviceProfileSuffix == UserChosenDeviceProfileSuffix; });
	...
	if (GIsEditor)
	{
		const UCWRPlatformEmulationSettings* Settings = GetDefault<UCWRPlatformEmulationSettings>();
	...
	{
	case ECWRFramePacingMode::MobileStyle:
		UpdateMobileFramePacing();
		break;
	case ECWRFramePacingMode::ConsoleStyle:
		UpdateConsoleFramePacing();
		break;
	case ECWRFramePacingMode::DesktopStyle:
		UpdateDesktopFramePacing();
		break;
	}
}

void UCWRSettingsLocal::UpdateConsoleFramePacing()
{
...
void UCWRSettingsLocal::UpdateDesktopFramePacing()
{
...
void UCWRSettingsLocal::UpdateMobileFramePacing()
{
...
void UCWRSettingsLocal::UpdateDynamicResFrameTime(float TargetFPS)
{
