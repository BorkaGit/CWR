...
//#include "CustomSettings/CWRSettingValueDiscrete_Language.h"
#include "Settings/CWRSettingsLocal.h"
#include "GameSettingValueDiscreteDynamic.h"
#include "Player/CWRLocalPlayer.h"
//#include "Replays/CWRReplaySubsystem.h"

#define LOCTEXT_NAMESPACE "CWR"

UGameSettingCollection* UCWRGameSettingRegistry::InitializeGameplaySettings(UCWRLocalPlayer* InLocalPlayer)
{
...

	{
		...
		//----------------------------------------------------------------------------------
		{
			/*UCWRSettingValueDiscrete_Language* Setting = NewObject<UCWRSettingValueDiscrete_Language>();
...

	{
		...
		//----------------------------------------------------------------------------------
		{
			...
			Setting->SetDefaultValue(GetDefault<UCWRSettingsLocal>()->ShouldAutoRecordReplays());

			Setting->AddEditCondition(FWhenPlayingAsPrimaryPlayer::Get());
			//Setting->AddEditCondition(FWhenPlatformHasTrait::KillIfMissing(UCWRReplaySubsystem::GetPlatformSupportTraitTag(), TEXT("Platform does not support saving replays")));
		...
		//----------------------------------------------------------------------------------
		{
			...
			Setting->SetDefaultValue(GetDefault<UCWRSettingsLocal>()->GetNumberOfReplaysToKeep());
