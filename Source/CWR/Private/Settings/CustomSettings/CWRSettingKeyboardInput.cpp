...
#include "Settings/CustomSettings/CWRSettingKeyboardInput.h"
...
namespace CWR::ErrorMessages
{
	static const FText UnknownMappingName = LOCTEXT("CWRErrors_UnknownMappingName", "Unknown Mapping");
}


UCWRSettingKeyboardInput::UCWRSettingKeyboardInput()
{
	bReportAnalytics = false;
}

FText UCWRSettingKeyboardInput::GetSettingDisplayName() const
{
	...
	return CWR::ErrorMessages::UnknownMappingName;
}

FText UCWRSettingKeyboardInput::GetSettingDisplayCategory() const
{
	if (const FKeyMappingRow* Row = FindKeyMappingRow())
	{
		if (Row->HasAnyMappings())
...
const FKeyMappingRow* UCWRSettingKeyboardInput::FindKeyMappingRow() const
{
...
UEnhancedPlayerMappableKeyProfile* UCWRSettingKeyboardInput::FindMappableKeyProfile() const
{
...
UEnhancedInputUserSettings* UCWRSettingKeyboardInput::GetUserSettings() const
{
	if(UCWRLocalPlayer* CWRLocalPlayer = Cast<UCWRLocalPlayer>(LocalPlayer))
	{
		// Map the key to the player key profile
		if (UEnhancedInputLocalPlayerSubsystem* System = CWRLocalPlayer->GetSubsystem<UEnhancedInputLocalPlayerSubsystem>())
		{
...
void UCWRSettingKeyboardInput::OnInitialized()
{
...
void UCWRSettingKeyboardInput::InitializeInputData(const UEnhancedPlayerMappableKeyProfile* KeyProfile, const FKeyMappingRow& MappingData, const FPlayerMappableKeyQueryOptions& InQueryOptions)
{
	check(KeyProfile);
...
FText UCWRSettingKeyboardInput::GetKeyTextFromSlot(const EPlayerMappableKeySlot InSlot) const
{
	if (const UEnhancedPlayerMappableKeyProfile* Profile = FindMappableKeyProfile())
	{
...
void UCWRSettingKeyboardInput::ResetToDefault()
{	
	if (UEnhancedInputUserSettings* Settings = GetUserSettings())
	{
...
void UCWRSettingKeyboardInput::StoreInitial()
{
	if (const UEnhancedPlayerMappableKeyProfile* Profile = FindMappableKeyProfile())
	{
		if(const FKeyMappingRow* Row = FindKeyMappingRow())
...
void UCWRSettingKeyboardInput::RestoreToInitial()
{
...
bool UCWRSettingKeyboardInput::ChangeBinding(int32 InKeyBindSlot, FKey NewKey)
{
...
void UCWRSettingKeyboardInput::GetAllMappedActionsFromKey(int32 InKeyBindSlot, FKey Key, TArray<FName>& OutActionNames) const
{	
	if (const UEnhancedPlayerMappableKeyProfile* Profile = FindMappableKeyProfile())
	{		
...
bool UCWRSettingKeyboardInput::IsMappingCustomized() const
{
	bool bResult = false;
