...
#include "UI/Frontend/CWRFrontendStateComponent.h"
...
UCWRFrontendStateComponent::UCWRFrontendStateComponent(const FObjectInitializer& ObjectInitializer)
	: Super(ObjectInitializer)
{
}

void UCWRFrontendStateComponent::BeginPlay()
{
	Super::BeginPlay();
...
void UCWRFrontendStateComponent::EndPlay(const EEndPlayReason::Type EndPlayReason)
{
	Super::EndPlay(EndPlayReason);
}

bool UCWRFrontendStateComponent::ShouldShowLoadingScreen(FString& OutReason) const
{
...
void UCWRFrontendStateComponent::OnExperienceLoaded(const UCWRExperienceDefinition* Experience)
{
...
void UCWRFrontendStateComponent::FlowStep_WaitForUserInitialization(FControlFlowNodeRef SubFlow)
{
...
void UCWRFrontendStateComponent::FlowStep_TryShowPressStartScreen(FControlFlowNodeRef SubFlow)
{
	...
	if (!UserSubsystem->ShouldWaitForStartInput())
	{
		...
		UserSubsystem->OnUserInitializeComplete.AddDynamic(this, &UCWRFrontendStateComponent::OnUserInitialized);
	...
	if (UPrimaryGameLayout* RootLayout = UPrimaryGameLayout::GetPrimaryGameLayoutForPrimaryPlayer(this))
	{
		...
		RootLayout->PushWidgetToLayerStackAsync<UCommonActivatableWidget>(FCWRGameplayTags::Get().UI_Layer_Menu, bSuspendInputUntilComplete, PressStartScreenClass,
...
void UCWRFrontendStateComponent::OnUserInitialized(const UCommonUserInfo* UserInfo, bool bSuccess, FText Error, ECommonUserPrivilege RequestedPrivilege, ECommonUserOnlineContext OnlineContext)
{
	...
	{
		UserSubsystem->OnUserInitializeComplete.RemoveDynamic(this, &UCWRFrontendStateComponent::OnUserInitialized);
...
void UCWRFrontendStateComponent::FlowStep_TryJoinRequestedSession(FControlFlowNodeRef SubFlow)
{
...
void UCWRFrontendStateComponent::FlowStep_TryShowMainScreen(FControlFlowNodeRef SubFlow)
{
	if (UPrimaryGameLayout* RootLayout = UPrimaryGameLayout::GetPrimaryGameLayoutForPrimaryPlayer(this))
	{
		constexpr bool bSuspendInputUntilComplete = true;
		RootLayout->PushWidgetToLayerStackAsync<UCommonActivatableWidget>(FCWRGameplayTags::Get().UI_Layer_Menu, bSuspendInputUntilComplete, MainScreenClass,
			[this, SubFlow](EAsyncWidgetLayerState State, UCommonActivatableWidget* Screen) {
			switch (State)
			{
			case EAsyncWidgetLayerState::AfterPush:
