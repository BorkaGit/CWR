...
#include "UI/Weapons/CWRReticleWidgetBase.h"

#include "Inventory/CWRInventoryItemInstance.h"
#include "Weapons/CWRRangedWeaponInstance.h"
#include "Weapons/CWRWeaponInstance.h"

UCWRReticleWidgetBase::UCWRReticleWidgetBase(const FObjectInitializer& ObjectInitializer)
	: Super(ObjectInitializer)
{
}

void UCWRReticleWidgetBase::InitializeFromWeapon(UCWRWeaponInstance* InWeapon)
{
	WeaponInstance = InWeapon;
	InventoryInstance = nullptr;
	if (WeaponInstance)
	{
		InventoryInstance = Cast<UCWRInventoryItemInstance>(WeaponInstance->GetInstigator());
	}
	OnWeaponInitialized();
}


float UCWRReticleWidgetBase::ComputeSpreadAngle() const
{
	if (const UCWRRangedWeaponInstance* RangedWeapon = Cast<const UCWRRangedWeaponInstance>(WeaponInstance))
	{
...
bool UCWRReticleWidgetBase::HasFirstShotAccuracy() const
{
	if (const UCWRRangedWeaponInstance* RangedWeapon = Cast<const UCWRRangedWeaponInstance>(WeaponInstance))
	{
...
float UCWRReticleWidgetBase::ComputeMaxScreenspaceSpreadRadius() const
{
